...
{
    private static readonly Lazy<FrozenDictionary<string, string>> roslynToLspMap = new(CreateRoslynToLspMap);
    private static readonly Lazy<FrozenDictionary<string, int>> lspToIndexMap = new(CreateLspToIndexMap);
    private static readonly Lazy<FrozenDictionary<string, string>> roslynToLspModifierMap = new(CreateRoslynToLspModifierMap);
    private static readonly Lazy<FrozenDictionary<string, int>> lspModifierToIndexMap = new(CreateLspModifierToIndexMap);

    public static FrozenDictionary<string, string> RoslynToLspMap => roslynToLspMap.Value;

    public static FrozenDictionary<string, int> LspToIndexMap => lspToIndexMap.Value;

    public static FrozenDictionary<string, string> RoslynToLspModifierMap => roslynToLspModifierMap.Value;

    public static FrozenDictionary<string, int> LspModifierToIndexMap => lspModifierToIndexMap.Value;

    private static FrozenDictionary<string, string> CreateRoslynToLspMap()
    {
        return ClassificationTypeNames.AllTypeNames
            .Where(static name => !ClassificationTypeNames.AdditiveTypeNames.Contains(name))
            .Select(static name => (name, value: MapFromRoslynToLsp(name)))
            .Where(static t => t.value != null)
            .ToFrozenDictionary(
                static t => t.name,
                static t => t.value!);
    }

    private static FrozenDictionary<string, int> CreateLspToIndexMap()
    {
        return RoslynToLspMap
            .Values
    ...
    private static FrozenDictionary<string, string> CreateRoslynToLspModifierMap()
    {
        return ClassificationTypeNames.AdditiveTypeNames
            .Select(static name => (name, value: MapFromRoslynToLspModifier(name)))
            .Where(static t => t.value != null)
            .ToFrozenDictionary(
                static t => t.name,
                static t => t.value!);
    ...
    private static FrozenDictionary<string, int> CreateLspModifierToIndexMap()
    {
        return RoslynToLspModifierMap
            .Values
            .Select(static (value, index) => (value, index))
            .ToFrozenDictionary(
                static t => t.value,
    ...
    private static string? MapFromRoslynToLsp(string name)
    {
        return name switch
        {
